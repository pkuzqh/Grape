...
class SumDataset(data.Dataset):
    def __init__(self, config, dataName="train"):
        ...
        self.ruledict["start -> Lambda"] = len(self.ruledict)
    ...
    def init_dic(self):
        ...
        #print(self.Code_Voc)
        assert("lambda" in self.Code_Voc)
    ...
    def preProcessData(self, dataFile):
        ...
        for i in tqdm(range(int(len(lines) / 5))):
            ...
            for j in range(min(self.Nl_Len, len(nl))):
                for k in range(min(self.Nl_Len, len(nl))):
                    inputad[j, k] = 1
            inputrule = [self.ruledict["start -> Lambda"]]
        ...
        batchs = [inputNl, inputNlChar, inputRule, inputRuleParent, inputRuleChild, inputRes, inputParent, inputParentPath, inputDepth, inputParentList]
        self.data = batchs
        self.nls = nls
        #self.code = codes
