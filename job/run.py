...
def train():
    ...
    for epoch in range(100000):
        ...
        for dBatch in tqdm(data_loader):
            if j % 3000 == 0:
                devloader = torch.utils.data.DataLoader(dataset=dev_set, batch_size=70,
                                              shuffle=False, drop_last=True, num_workers=1)
                model = model.eval()
                accs = []
                tcard = []
                tmp = []
                antimask2 = antimask.unsqueeze(0).repeat(70, 1, 1).unsqueeze(1)
                rulead2 = rulead.unsqueeze(0).repeat(2, 1, 1)
                for devBatch in tqdm(devloader):
                    for i in range(len(devBatch)):
                        devBatch[i] = gVar(devBatch[i])
                    with torch.no_grad():
                        _, pre = model(devBatch[0], devBatch[1], devBatch[2], devBatch[3], devBatch[4], devBatch[6], devBatch[7], devBatch[8], devBatch[9], tmpindex, tmpf, tmpc, rulead2, antimask2, devBatch[5])
            ...
            loss, _ = model(dBatch[0], dBatch[1], dBatch[2], dBatch[3], dBatch[4], dBatch[6], dBatch[7], dBatch[8], dBatch[9], tmpindex, tmpf, tmpc, rulead2, antimask2, dBatch[5])
...
class SearchNode:
    def __init__(self, ds, nl):
        ...
        for j in range(min(ds.Nl_Len, len(nl))):
            for k in range(min(ds.Nl_Len, len(nl))):
                self.parent[j, k] = 1
    ...
    def applyrule(self, rule, nl, ds):
        ...
        if self.expanded.father:
            ...
            ids = 1
        tmpdepth.append(ids)
        tmpdepth.extend(self.depth[self.expanded.fatherlistID])
...
def BeamSearch(inputnl, vds, model, beamsize, batch_size, k):
    ...
    with torch.no_grad():
        ...
        for i in range(batch_size):
            beams[i] = [SearchNode(vds, vds.nl[batch_size * k + i])]
        index = 0
        antimask = gVar(getAntiMask(args.CodeLen))
        endnum = {}
        continueSet = {}
        while True:
            ...
            for p in range(beamsize):
                ...
                result = model(gVar(inputnl[0][validnum]), gVar(inputnl[1][validnum]), gVar(tmprule), gVar(tmpruleparent), gVar(tmprulechild), gVar(tmpAd), gVar(tmptreepath), gVar(tmpdepth), None, tmpindex, tmpf, tmpc, rulead, antimasks, None, "test")
                ...
                for j in range(args.batch_size):
                    ...
                    for i in range(tmpbeamsize):
                        ...
                        c = copynode.applyrule(indexs[i], vds.nl[args.batch_size * k + j], vds)
                        if not c:
                            tmpbeamsize += 1
                            continue
